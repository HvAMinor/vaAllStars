...
# 0. DATA OPHALEN, SCHOONMAKEN, FILTER, SELECTEREN, JOINEN ---------
# 
...
bronnen <- 
  paste(
    ...
    "--------------",
    "GITHUB (2019) Rstudio Github ShinyDashboard [Data file] Retrieved from: https://rstudio.github.io/shinydashboard/",
    "Stackoverflow (2019) Stackoverflow ShinyDashboard [Data file] Retrieved from: https://stackoverflow.com/questions/tagged/shinydashboard",
    sep="\n"
  )
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem(text = "Bronnenlijst", tabName = "bronnenlijst")
    )
  ),
  ## sidebar close
  ...
  dashboardBody(tabItems(
    ...
    # `-------- bronnenlijst ---------
    tabItem(tabName = "bronnenlijst",
            fluidRow(box(title = "de bronnenlijst", width=12, verbatimTextOutput("bronnenlijst.meta")))
    ),
...
server <- function(input, output) {
    output$bronnenlijst.meta <- renderText({
      bronnen
    })
